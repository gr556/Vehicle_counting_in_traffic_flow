...
# # video_out = cv2.VideoWriter("output_3lanes_lane_numbers.mp4", fourcc, fps, (width, height))
...
# #         # Draw ID, movement arrow, and lane number
...
# #     cv2.imshow("Traffic Flow - 3 Lanes with Lane Numbers", frame)
...
# # ===================== VIDEO WRITER WITH TIMESTAMP =====================
# timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
# output_file = f"vehicle_count_{timestamp}.mp4"
# fourcc = cv2.VideoWriter_fourcc(*"mp4v")
...
#         color=(0,0,0)
#         cv2.rectangle(frame,(x,y),(x+w,y+h),color,2)
#         # Draw ID, lane number, and movement arrow
#         cv2.putText(frame,f"ID {matched_id}",(x,y-5),cv2.FONT_HERSHEY_SIMPLEX,0.5,color,2)
#         cv2.putText(frame,f"Lane {lane}",(x,y-20),cv2.FONT_HERSHEY_SIMPLEX,0.6,lane_colors[lane],2)
#         cv2.arrowedLine(frame,(cx,cy),(cx,cy-15),(0,0,255),2)
